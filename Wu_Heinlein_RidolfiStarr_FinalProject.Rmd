...
* Figure \@ref(fig-map): Locations of Water Quality Sampling Sites
...
Unprocessed observations were plotted in Figure \@ref(fig:points) to identify extreme outliers. Several impossible values are evident in the dissolved oxygen and pH data. 
...
Extreme outliers representing impossible values (dissolved oxygen \> 1000 and pH \> 15) were identified visually using Figure \@ref(fig:points) and replaced with NA values. The remaining data likely still contain less-extreme outliers. 
...
Descriptive statistics of the study parameters (with extreme outliers removed) are summarized in Table \@ref(tab:table1).
...
\
\
```{r Pyearlyvsmonthly, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Comparing Yearly Averages with Interpolated Monthly Averages: Phosphorous"}
...
\
\
```{r Nyearlyvsmonthly, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Comparing Yearly Averages with Interpolated Monthly Averages: Nitrogen"}
#Nitrogen
