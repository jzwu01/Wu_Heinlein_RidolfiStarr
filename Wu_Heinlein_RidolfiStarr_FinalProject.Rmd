...
WQ_parameters_treated <- WQ_parameters_raw  %>%
  mutate(
    ...
    Dissolved_O2.fixed = ifelse(Dissolved_O2>700,NA,Dissolved_O2))
...
P.points <- ggplot() +
  geom_point(data=WQ_parameters_treated, aes(x=SampleDate, y=Total_Phosphorous))
N.points <- ggplot() +
  geom_point(data=WQ_parameters_treated, aes(x=SampleDate, y=Total_Nitrogen))
O.points <- ggplot() +
  geom_point(data=WQ_parameters_treated, aes(x=SampleDate, y=Dissolved_O2))
pH.points <- ggplot() +
  geom_point(data=WQ_parameters_treated, aes(x=SampleDate, y=pH))

plot_grid(P.points, N.points, O.points, pH.points, nrow=2, ncol=2)
```
...
kable(mk.summary,
      digits=c(3,3),
      caption="Results Summary of Mann Kendall Test on Yearly Averages")
```
